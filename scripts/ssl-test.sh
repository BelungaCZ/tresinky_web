...
for dir in "${SSL_DIRS[@]}"; do
    if [ -d "./ssl-data/$dir" ]; then
        ...
        # Check if mounted in nginx-proxy (skip acme directory for nginx-proxy)
        if [ "$dir" != "acme" ]; then
            ...
            print_info "./ssl-data/$dir (not mounted in nginx-proxy - correct behavior)"
        fi
...
if docker compose config | grep -q "ssl-data/certs" && docker compose config | grep -q "/etc/nginx/certs"; then
    print_step "nginx-proxy SSL certs volume configured"
else
    print_error "nginx-proxy SSL certs volume missing"
fi

if docker compose config | grep -q "ssl-data/acme" && docker compose config | grep -q "/etc/acme.sh"; then
